...
For the remaining three labs, we will focus on making thematic maps using GeoJSON data. To begin this lab, open the *index.html* the *lab-08/* directory and follow the tutorial and instructions below, saving your work and committing changes to Git as you go.
...
Run through **Part I** in the lesson _README.md_ and use the _lesson-08/index.html_ file to write/execute the statements presented in the lesson. Save a copy of your current *index.html* file as *index-v1.html* and commit changes to Git. You should show at least 50% of the lesson example statements. You can of course comment out statements that you're already completed. Commit changes as you go and finally push the *index-v1.html* to your GitHub repo.

## Part II. Storing and loading GeoJSON data as an external file (**2 pt**)

Run through **Part II** in the lesson _README.md_ and add the code to the _lesson-08/index.html_ file to write/execute the statements presented in the lesson. You'll need to write 100% of the statements. Save a copy of your current *index.html* file and commit changes to Git.
...
A second solution, which we can couple with stacking them with larger ones on the bottom, is to reduce the opacity of the circles' fill while retaining a high opacity of the stroke. We achieve this easily using [Leaflet's Path options](http://leafletjs.com/reference-1.3.0.html#path):
...
To bind a user-generated interaction to these layers, we're going to be using the Leaflet *Path* events `mouseover` and `mouse out` ([http://leafletjs.com/reference-1.3.0.html#path](http://leafletjs.com/reference-1.3.0.html#path)), which are triggered by using Leaflet's `.on()` method:
